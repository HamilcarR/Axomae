...
GLFrameBuffer::GLFrameBuffer(unsigned _width,
                             unsigned _height,
                             GLRenderBuffer::INTERNAL_FORMAT rbo_format,
                             unsigned int *default_fbo_id_pointer,
                             TEXTURE_TARGET target_type)
    : GLFrameBuffer() {
...
void GLFrameBuffer::attachTexture2D(INTERNAL_FORMAT color_attachment,
...
void GLFrameBuffer::initializeBuffers() {
  ...
  if (renderbuffer_object != nullptr) {
    ...
    if (status != GL_FRAMEBUFFER_COMPLETE) {
      LOG("Framebuffer not ready to use " + std::to_string(status), LogLevel::ERROR);
    }
  }
  unbind();
}
