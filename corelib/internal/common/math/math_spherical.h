...
namespace math::spherical {
  ...
  template<class T>
  ax_device_callable glm::dvec2 uvToSpherical(const T &u, const T &v) {
    const T phi = 2 * PI * u;
    const T theta = PI * v;
    return glm::dvec2(phi, theta);
  }

  ax_device_callable_inlined glm::dvec2 uvToSpherical(const glm::dvec2 &uv) { return uvToSpherical(uv.x, uv.y); }
  ...
  ax_device_callable_inlined glm::dvec2 sphericalToUv(const glm::dvec2 &sph) { return sphericalToUv(sph.x, sph.y); }

  template<class T>
  ax_device_callable_inlined glm::dvec3 sphericalToCartesian(const T &phi, const T &theta) {
    const T z = cos(theta);
  ...
  ax_device_callable_inlined glm::dvec3 sphericalToCartesian(const glm::dvec2 &sph) { return sphericalToCartesian(sph.x, sph.y); }

  template<class T>
  ax_device_callable_inlined glm::dvec2 cartesianToSpherical(const T &x, const T &y, const T &z, bool fast_approx = true) {
  ...
  ax_device_callable_inlined glm::dvec2 cartesianToSpherical(const glm::dvec3 &xyz, bool fast_approx = true) {
  ...
  ax_device_callable_inlined SphAxis cartesianToSphericalAxis(const glm::dvec3 &xyz) {
