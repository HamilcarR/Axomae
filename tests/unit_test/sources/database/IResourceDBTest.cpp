...
class TestDatabase : public IntegerResourceDB<TestString> {
 public:
  ...
  explicit TestDatabase(axstd::MemoryArena<std::byte> *arena) { setUpCacheMemory(arena); }
};
...
TEST(IResourceDB, reserveCache) {
  for (uint32_t test_i = 0; test_i < NUM_TESTS; test_i++) {
    ...
    axstd::MemoryArena arena(block_size_arena);
    TestDatabase test_database(&arena);
    auto ptr = test_database.reserveCache(ALLOC_SIZE, axstd::PLATFORM_ALIGN);
...
TEST(IResourceDB, getCacheSize) {
  ...
  for (uint32_t i = 0; i < NUM_TESTS; i++) {
    std::size_t block_size_arena = generator.nrandi(2048, 65556);
    uint8_t *ptr = nullptr;
    ASSERT_NE(ptr = test_database.reserveCache(block_size_arena, axstd::PLATFORM_ALIGN), nullptr);
...
TEST(IResourceDB, invalidateCaches) {
  ...
  for (uint32_t i = 0; i < NUM_TESTS; i++) {
    std::size_t block_size_arena = generator.nrandi(2048, 65556);
    ASSERT_NE(test_database.reserveCache(block_size_arena, axstd::PLATFORM_ALIGN), nullptr);
  }
...
static void verify_out_of_bounds() {
  axstd::MemoryArena arena;
  TestDatabase test_database(&arena);
  uint8_t *cache = test_database.reserveCache(sizeof(TestString), axstd::PLATFORM_ALIGN);
...
void add_string() {
  axstd::MemoryArena arena;
  TestDatabase test_database(&arena);
  uint8_t *cache = test_database.reserveCache(ALLOC_SIZE, axstd::PLATFORM_ALIGN);
