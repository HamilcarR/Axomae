...
namespace axomae {
  ...
  void async_copy_buffer(unsigned int width, unsigned int height, uint8_t *from, uint8_t *dest) {
    ...
    size_t thread_buffer_size =
        total_buffer_size / THREAD_POOL_SIZE;  // TODO : implement the case where total_buffer_sze < THREAD_POOL_SIZE
    ...
    if (ai_node != nullptr) {
      ...
      if (ai_node->mNumMeshes == 0) {
        ...
        add_node.push_back(SceneNodeBuilder::buildEmptyNode(
            parent));  // Little compatibility hack between assimp and the node system, assimp nodes can contain
                       // multiples meshes , but SceneTreeNode can be a mesh.
  ...
  std::pair<unsigned, Object3D *> geometry_fill_buffers(const aiScene *modelScene, unsigned i) {
    ...
    std::for_each(
        shared_futures.begin(), shared_futures.end(), [](std::shared_future<void> &it) -> void { it.wait(); });
  ...
  std::pair<std::vector<Mesh *>, SceneTree> Loader::loadObjects(
    ...
    if (modelScene != nullptr) {
      ...
      for (auto it = loaded_meshes_futures.begin(); it != loaded_meshes_futures.end(); it++) {
        ...
        Mesh *loaded_mesh = static_cast<Mesh *>(SceneNodeBuilder::buildMesh(
            nullptr, name, std::move(*geometry), material_array[mesh_index], shader_program));
