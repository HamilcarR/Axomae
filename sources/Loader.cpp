...
void async_copy_buffer(unsigned int width, unsigned int height, uint8_t *from, uint8_t *dest) {
  ...
  size_t thread_buffer_size = total_buffer_size /
                              THREAD_POOL_SIZE;  // TODO : implement the case where total_buffer_sze < THREAD_POOL_SIZE
...
static void copyTexels(TextureData *totexture, aiTexture *fromtexture) {
  if (totexture != nullptr) {
    ...
    else
    {
      ...
      LOG("image of size " + std::to_string(totexture->width) + " x " + std::to_string(totexture->height) +
              " uncompressed ",
          LogLevel::INFO);
    }
  }
...
static Material loadAllTextures(const aiScene *scene, const aiMaterial *material, TextureDatabase *texture_database) {
  ...
  aiString color_texture, opacity_texture, normal_texture, metallic_texture, roughness_texture, emissive_texture,
 ...
 */
ISceneNode *fillTreeData(aiNode *ai_node,
                         const std::vector<Mesh *> &mesh_lookup,
                         ISceneNode *parent,
  ...
  if (ai_node != nullptr) {
    ...
    if (ai_node->mNumMeshes == 0) {
      ...
      add_node.push_back(SceneNodeBuilder::buildEmptyNode(
          parent));  // Little compatibility hack between assimp and the node system, assimp nodes can contain multiples
                     // meshes , but SceneTreeNode can be a mesh.
      node_deletion.push_back(
          add_node[0]);  // So we create a dummy node at position 0 in add_node to be the ancestors of the children
...
std::pair<std::vector<Mesh *>, SceneTree> Loader::loadObjects(
  ...
  if (modelScene != nullptr) {
    ...
    for (auto it = loaded_meshes_futures.begin(); it != loaded_meshes_futures.end(); it++) {
      ...
      Mesh *loaded_mesh = static_cast<Mesh *>(
          SceneNodeBuilder::buildMesh(nullptr, name, std::move(*geometry), material_array[mesh_index], shader_program));
...
Mesh *Loader::generateCubeMap(bool is_glb) {
  ...
  auto thread_lambda_func =
    ...
    for (unsigned y = 0; y < height; y++) {
      const QRgb *line = reinterpret_cast<const QRgb *>(
          array[i].scanLine(y));  // Since the cubemaps don't have alpha channel , we interpret the data as RGB
  ...
  for (unsigned i = 0; i < array.size(); i++)
    threads_future.push_back(std::async(thread_lambda_func, i, pointer_on_cubemap_data, cubemap.width, cubemap.height));
