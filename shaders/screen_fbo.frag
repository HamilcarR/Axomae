...
vec3 computeReinhardToneMapping(vec3 hdr_color){
    return hdr_color.rgb / (hdr_color.rgb + vec3(1.f)); 
}

vec3 computeExposureToneMapping(vec3 hdr_color){
    return vec3(1.f) - exp(-hdr_color * exposure); 
}
