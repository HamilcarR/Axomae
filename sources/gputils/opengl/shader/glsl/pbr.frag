...
vec4 computeAmbiantOcclusionValue() { return texture(ambiantocclusion_map, vertex_fragment_uv); }

vec4 computeRoughnessValue() { return texture(roughness_map, vertex_fragment_uv); }

vec4 computeEmissiveValue() { return texture(emissive_map, vertex_fragment_uv); }

vec4 computeMetallicValue() { return texture(metallic_map, vertex_fragment_uv); }

vec4 computeDiffuseValue() { return texture(diffuse_map, vertex_fragment_uv); }

vec4 computeIrradiance(vec3 normal) { return texture(irradiance_map, normal); }
...
vec3 fresnelSchlickGGX(float cos_theta, float metallic, vec3 albedo, vec3 F0) { return F0 + (1.f - F0) * pow(clamp(1.f - cos_theta, 0.f, 1.f), 5.f); }

vec3 computeFresnel(vec3 h, vec3 v, float metallic, vec3 albedo, vec3 F0) { return fresnelSchlickGGX(max(dot(h, v), 0.f), metallic, albedo, F0); }
