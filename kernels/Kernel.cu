...
public:
	...
	template<typename T> __device__
		DEVICE_RGB operator*(T value) {
		return  { uint8_t(r * value) ,uint8_t(g * value) ,uint8_t(b * value) ,uint8_t(a * value) };
	}
	__device__
		DEVICE_RGB normalize_rgb(DEVICE_RGB max , DEVICE_RGB min) {
	...
	__device__
		 DEVICE_RGB magnitude_rgb(DEVICE_RGB horizontal , DEVICE_RGB vertical) {
		DEVICE_RGB rgb; 
...
/*********************************************************************************************************************************************/
__host__ __device__
	uint32_t rgb_to_int(DEVICE_RGB val) {
...
__device__
	DEVICE_RGB compute_greyscale(DEVICE_RGB rgb, const bool luminance) {
	DEVICE_RGB ret;
	if (luminance) {
...
__device__
	DEVICE_RGB int_to_rgb(uint8_t* pixel_value, const int bpp) {
...
__device__
	void set_pixel_color(uint8_t* pixel_value, DEVICE_RGB rgb, const int bpp) {
...
__device__
	DEVICE_RGB get_pixel_value_at(uint8_t *pixel, int i, int j, const int bpp, int pitch) {
...
//TODO : case kernel < 0 
__device__ convolution_directions compute_convolution(uint8_t * pixel, const int bpp, int pitch, const int h_kernel[KERNEL_SIZE][KERNEL_SIZE], const int v_kernel[KERNEL_SIZE][KERNEL_SIZE], uint8_t border_flag) {
...
__global__ void GPU_compute_edges(void* image, void* save, unsigned int width, unsigned int height, int bpp, int pitch, uint8_t convolution, uint8_t border) {
...
__global__ void GPU_compute_normals(void* image, void* save, unsigned int width, unsigned int height, int bpp, int pitch, double factor, uint8_t border) {
	int i = blockDim.x * blockIdx.x + threadIdx.x; 
	int j = blockDim.y * blockIdx.y + threadIdx.y; 
	uint8_t* pixel = (uint8_t*)(image)+i * bpp + j * pitch; 
