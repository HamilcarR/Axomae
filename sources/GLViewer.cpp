...
void GLViewer::paintGL(){
	...
	if(start_draw && shader_program && current_scene){
		...
		{ 
			...
			vao.bind();		
			errorCheck() ;  
			glDrawElements(GL_TRIANGLES , current_scene->geometry.indices.size() , GL_UNSIGNED_INT , 0 );
			vao.release();
			shader_program->release();		
		}
	}
...
void GLViewer::setNewScene(std::vector<Mesh> &new_scene){
	...
	*current_scene = new_scene[0];
	...
	glTexImage2D(GL_TEXTURE_2D , 0 , GL_RGBA , current_scene->material.textures.diffuse.width , current_scene->material.textures.diffuse.height , 0 , 
			GL_BGRA , GL_UNSIGNED_BYTE , current_scene->material.textures.diffuse.data); 
