...
namespace nova::shape {
  ...
  class Triangle {
   ...
   public:
    ...
    ax_device_callable face_data_s getTransformedFace(const MeshCtx &geometry) const {
      const Object3D &obj3d = geometry.getTriMesh(mesh_id);
    ...
    ax_device_callable_inlined transform::transform4x4_t getTransform(const MeshCtx &geometry) const { return geometry.getTriMeshTransform(mesh_id); }

    ax_device_callable Triangle(std::size_t mesh_id_, std::size_t triangle_id_) {
    ...
    ax_device_callable bool hasValidTangents(const MeshCtx &geometry) const {
      geometry::face_data_tri face = getFace(geometry).data.triangle_face;
      return face.hasValidTangents();
    }

    ax_device_callable bool hasValidBitangents(const MeshCtx &geometry) const {
      geometry::face_data_tri face = getFace(geometry).data.triangle_face;
      return face.hasValidBitangents();
    }

    ax_device_callable bool hasValidNormals(const MeshCtx &geometry) const {
      geometry::face_data_tri face = getFace(geometry).data.triangle_face;
      return face.hasValidNormals();
    }

    ax_device_callable bool hasValidUvs(const MeshCtx &geometry) const {
      geometry::face_data_tri face = getFace(geometry).data.triangle_face;
     ...
     */
    ax_device_callable bool hit(const Ray &in_ray, float tmin, float last_hit_tmax, hit_data &data, const MeshCtx &geometry) const {
      ...
      if (!hasValidNormals(geometry)) {
        ...
        data.normal = math::geometry::barycentric_lerp(normals.v0, normals.v1, normals.v2, w, u, v);
      }
      ...
      data.tangent = math::geometry::barycentric_lerp(tangents.v0, tangents.v1, tangents.v2, w, u, v);
      ...
      if (!hasValidBitangents(geometry)) {
        ...
        data.bitangent = math::geometry::barycentric_lerp(bitangents.v0, bitangents.v1, bitangents.v2, w, u, v);
